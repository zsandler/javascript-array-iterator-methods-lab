...
veryOldInventors.filter((inventor) => {
...
// Complete the exercise in the space below:
// Define the new array (inventorNamedAda =); 
// Apply the iterator (inventors.find(inventor))
// Return the specific value we're looking for
...
// // 1st attempt: showed Array(40) 40 times
...
// Complete the exercise in the space below:
// I think it's close other than a syntax error or a few?

isAdultPresent = devs.sort((dev) => {
    if (staticYear - year >= 18)
    return isAdultPresent = true;
    } else {
        return isAdultPresent = false
})
